...
export const ReviewDbChangeCallbacks: DbChangeCallbacks<ReviewFromModel, ReviewEntity> = {
	created: async ({ after }) => {
		await appInstance.listener.created(['jobs/reviews', `jobs/reviews/${after.id}`], after)
		await UsersUseCases.updateRatings({ userId: after.to, ratings: after.rating, add: true })
	},
	updated: async ({ after, before, changes }) => {
		await appInstance.listener.updated(['jobs/reviews', `jobs/reviews/${after.id}`], after)
	...
	deleted: async ({ before }) => {
		await appInstance.listener.deleted(['jobs/reviews', `jobs/reviews/${before.id}`], before)
		await UsersUseCases.updateRatings({ userId: before.to, ratings: before.rating, add: false })
	}
}